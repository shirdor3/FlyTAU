...
def report_avg_occupancy():
    ...
    """
    df = _query_df(sql)
    val = float(df.iloc[0]["avg_occupancy_percent"] or 0)
...
def report_staff_hours():
    ...
    FROM (
        ...
        WHERE f.status = 'ACTIVE' AND f.departure_datetime < NOW()
...
def report_cancellation_rate():
    ...
    SELECT YEAR(r.reservation_date) AS order_year, MONTH(r.reservation_date) AS order_month,
        (SUM(CASE WHEN r.reservations_status = 'CUSTOMER_CANCELED' THEN 1 ELSE 0 END) / COUNT(reservation_code) * 100) AS cancellation_rate
    FROM reservations AS r
